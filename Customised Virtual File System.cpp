...
void DisplayHelp()
{
    ...
    printf(" rm : To List out all files\n");
}
...
void InitialiseSuperBlock()
{
    ...
    SUPERBLOCKobj.TotolInodes = MAXINODE;
    SUPERBLOCKobj.FreeInode = MAXINODE;
}

int CreatFile(char *name, int permisison)
{
    int i = 3;
    PINODE temp = head;

    if((name == NULL) || (permisison == 0)||(permisison > 3))
        return -1;
    ...
    UFDTArr[i].ptrfiletable ->mode = permisison;
    ...
    UFDTArr[i].ptrfiletable->ptrinode->permission = permisison;
...
int rm_File(char * name)
{
    ...
    if(UFDTArr[fd].ptrfiletable->ptrinode->LinkCount == 0)
    {
          ...
          //free(UFDTArr[fd].ptrfiletable->ptrinode->Biffer);
...
int ReadFile(int fd, char *arr, int isize)
{
    ...
    if(UFDTArr[fd].ptrfiletable->readoffset != READ && UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize != READ + WRITE)   return -3;
                ...
                if (ret == -1)
                  ...
                  if(ret == 0)
                    ...
                    printf("ERROR : Permission denied\n");
                     if (ret == -2)
                    printf("ERROR : There is no sufficient momory to write\n");
                    if (ret == -3)
